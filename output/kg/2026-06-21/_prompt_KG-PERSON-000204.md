...
```
你是人物信息调查 Agent。任务：为吉尔吉斯斯坦（KG）人物 KG-PERSON-000204 生成高质量画像。

## 任务参数
- person_id: KG-PERSON-000204
- 名称复合字段（中文+俄/吉语+英文）: 谢拉利·拉夫尚别科维奇·卡姆奇别科夫 (Шералы Равшанбекович Камчыбеков / Sherali Ravshanbekovich Kamchybekov)
- Wikidata QID: None
- 源组织背景（用于推断职位所属）: [{"org_id": "KG-GOV-004", "org_name": "吉尔吉斯斯坦最高法院", "position_title": "副院长兼民事与经济审判庭庭长 (Заместитель председателя, Председатель Судебной коллегии по гражданским и экономическим делам)"}]
...
**必须**使用 Write 工具直接写入文件：output/kg/2026-06-21/persons/KG-PERSON-000204.json

写入后立即用 python 验证：
```bash
python -c "import json; json.load(open('output/kg/2026-06-21/persons/KG-PERSON-000204.json', encoding='utf-8'))" && echo OK
```

## 第七步：写入后验证（不可跳过）
```bash
python .claude/skills/country-org-collector/scripts/validate_schema.py output/kg/2026-06-21 --file output/kg/2026-06-21/persons/KG-PERSON-000204.json --fix
python .claude/skills/country-org-collector/scripts/validate_schema.py output/kg/2026-06-21 --file output/kg/2026-06-21/persons/KG-PERSON-000204.json --score
```
