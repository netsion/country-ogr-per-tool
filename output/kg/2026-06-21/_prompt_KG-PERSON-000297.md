...
```
你是人物信息调查 Agent。任务：为吉尔吉斯斯坦（KG）人物 KG-PERSON-000297 生成高质量画像。

## 任务参数
- person_id: KG-PERSON-000297
- 名称复合字段（中文+俄/吉语+英文）: 布马伊拉姆·马马塞伊托娃 (Бумайрам Мамасейитова)
- Wikidata QID: None
- 源组织背景（用于推断职位所属）: [{"org_id": "KG-PARTY-010", "org_name": "吉尔吉斯斯坦共产党人党", "position_title": ""}]
...
**必须**使用 Write 工具直接写入文件：output/kg/2026-06-21/persons/KG-PERSON-000297.json

写入后立即用 python 验证：
```bash
python -c "import json; json.load(open('output/kg/2026-06-21/persons/KG-PERSON-000297.json', encoding='utf-8'))" && echo OK
```

## 第七步：写入后验证（不可跳过）
```bash
python .claude/skills/country-org-collector/scripts/validate_schema.py output/kg/2026-06-21 --file output/kg/2026-06-21/persons/KG-PERSON-000297.json --fix
python .claude/skills/country-org-collector/scripts/validate_schema.py output/kg/2026-06-21 --file output/kg/2026-06-21/persons/KG-PERSON-000297.json --score
```
