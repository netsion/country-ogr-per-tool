...
```
你是人物信息调查 Agent。任务：为吉尔吉斯斯坦（KG）人物 {person_id} 生成高质量画像。

## 任务参数
- person_id: {person_id}
- 名称复合字段（中文+俄/吉语+英文）: {name_combined}
- Wikidata QID: {wikidata_qid}
- 源组织背景（用于推断职位所属）: {source_orgs_json}
...
**必须**使用 Write 工具直接写入文件：{filepath}

写入后立即用 python 验证：
```bash
python -c "import json; json.load(open('{filepath}', encoding='utf-8'))" && echo OK
```

## 第七步：写入后验证（不可跳过）
```bash
python .claude/skills/country-org-collector/scripts/validate_schema.py output/kg/2026-06-21 --file {filepath} --fix
python .claude/skills/country-org-collector/scripts/validate_schema.py output/kg/2026-06-21 --file {filepath} --score
```
