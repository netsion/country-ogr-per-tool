...
Usage:
  python validate_schema.py output/kr/2026-04-28 [--fix] [--score] [--strict]
  ...
  --strict  Data-quality gate: escalate ironrule checks (event sources, related_entities
            org_id/relationship_type, key_people person_id, extra fields, empty-string IDs,
            self-references) to errors; cross-ref dangling refs and person registry
            listed/profiled-without-file also become errors. CJK naming rules only apply
            to CJK-region datasets (CN/HK/TW/MO) to avoid false positives on Latin datasets.
"""
...
# --- Strict mode globals (set in main()) ---
STRICT = False          # --strict: ironrule checks escalate to errors
DATASET_ISO = None      # e.g. 'HK' — derived from output dir path; drives CJK-region rules
CJK_RULE_REGIONS = {'CN', 'HK', 'TW', 'MO'}  # regions where Chinese-naming rules apply
...
# Load full schemas: derive allowed top-level keys from "properties" (no hardcoding)
def load_schemas():
    schemas = {}
    for key, fname in (("org", "org_profile_schema.json"), ("person", "person_profile_schema.json")):
...
ORG_TOP_FIELDS = set(SCHEMAS.get("org", {}).get("properties", {}).keys())
PERSON_TOP_FIELDS = set(SCHEMAS.get("person", {}).get("properties", {}).keys())
# Legacy compatibility keys tolerated on person files (present in older data, not in schema)
PERSON_TOP_STRICT_ALLOW = PERSON_TOP_FIELDS | {'org_ids', 'name_ko'}

# Nested-array field whitelists (from rules handbook; strict mode flags anything else)
# note: key_people.name_en is an accepted supplementary convention in org key_people entries
KP_FIELDS = {'person_id', 'name', 'name_en', 'title', 'title_description', 'description'}
RELATED_FIELDS = {'org_id', 'org_name', 'org_type', 'org_description', 'relationship_type'}
REL_FIELDS = {'person_id', 'person_name', 'relationship_type', 'description'}
WORK_FIELDS = {'organization', 'org_id', 'position', 'start_date', 'end_date'}
EDU_FIELDS = {'institution', 'degree', 'field', 'start_date', 'end_date'}
...
def _bracket_reversed(name):
    """True if '本地语言 (中文)' — Chinese appears inside brackets but not before them.
    CJK-region rule: Chinese must come first: '中文 (English)'."""
    m = re.match(r'^([^（(]*)[(（]([^)）]*)[)）]\s*$', name or '')
    if not m:
        return False
    before, inside = m.group(1), m.group(2)
    return _has_cjk(inside) and not _has_cjk(before)


def _cjk_region_rules_apply():
    return DATASET_ISO in CJK_RULE_REGIONS
...
def normalize_data(data, is_person=True):
    ...
    if is_person:
        ...
        extra = set(data.keys()) - ORG_TOP_FIELDS
        for key in extra:
            del data[key]
            fixes.append(f"removed extra field '{key}'")
...
def save_json_file(fpath, data):
    # Atomic write (tempfile + fsync + os.replace) via shared helper
    sys.path.insert(0, SCRIPT_DIR)
    from atomic_write import atomic_json_write
    atomic_json_write(fpath, data)
...
def validate_org_profile(data, filepath):
    ...
    for i, kp in enumerate(data.get("key_people", [])):
        if not kp.get("name"):
            (r.error if STRICT else r.warn)(f"key_people[{i}].name is empty")
        if not kp.get("title"):
    ...
    # --- Strict-mode org checks (ironrule escalations) ---
    if STRICT:
        top_extra = set(data.keys()) - ORG_TOP_FIELDS
        for k in sorted(top_extra):
            r.error(f"strict: extra top-level field '{k}' not in org schema")
        if not (bi.get("name_zh") or "").strip():
            r.error("strict: basic_info.name_zh is empty")

        kp_allowed = KP_FIELDS
        for i, kp in enumerate(data.get("key_people", [])):
            extra = set(kp.keys()) - kp_allowed
            if extra:
                r.error(f"strict: key_people[{i}] extra fields {sorted(extra)}")
            if not (kp.get("person_id") or "").strip():
                r.error(f"strict: key_people[{i}].person_id is null/missing (every key person must resolve to a profile)")
            nm = kp.get("name") or ""
            if _cjk_region_rules_apply():
                if nm and not _has_cjk(nm):
                    r.error(f"strict: key_people[{i}].name '{nm}' has no Chinese (CJK region)")
                elif nm and _bracket_reversed(nm):
                    r.error(f"strict: key_people[{i}].name '{nm}' bracket direction reversed (expected 中文 (English))")

        rel_allowed = RELATED_FIELDS
        for i, re_ in enumerate(data.get("related_entities", [])):
            # org_id missing → strict WARNING only: datasets may deliberately keep
            # unregistered orgs as description-only references (approved downgrade)
            if not (re_.get("org_id") or "").strip():
                r.warn(f"strict: related_entities[{i}].org_id is null/missing (description-only reference)")
            if not (re_.get("relationship_type") or "").strip():
                r.error(f"strict: related_entities[{i}].relationship_type is null/missing")
            extra = set(re_.keys()) - rel_allowed
            if extra:
                r.error(f"strict: related_entities[{i}] extra fields {sorted(extra)}")

        for i, ev in enumerate(data.get("recent_events", [])):
            if not (ev.get("source") or "").strip():
                r.error(f"strict: recent_events[{i}].source is missing (every event needs a traceable source URL)")
...
def validate_person_profile(data, filepath):
    ...
    # --- Strict-mode person checks (ironrule escalations) ---
    if STRICT:
        top_extra = set(data.keys()) - PERSON_TOP_STRICT_ALLOW
        for k in sorted(top_extra):
            r.error(f"strict: extra top-level field '{k}' not in person schema")

        for i, p in enumerate(data.get("current_positions") or []):
            if isinstance(p, dict):
                r.error(f"strict: current_positions[{i}] is an object, expected a plain Chinese string")
            elif p is not None and not isinstance(p, str):
                r.error(f"strict: current_positions[{i}] expected string, got {type(p).__name__}")

        we_allowed = WORK_FIELDS
        for i, work in enumerate(data.get("work_experience", [])):
            extra = set(work.keys()) - we_allowed
            if extra:
                r.error(f"strict: work_experience[{i}] extra fields {sorted(extra)}")
            oid = work.get("org_id")
            if isinstance(oid, str):
                if not oid.strip():
                    r.error(f"strict: work_experience[{i}].org_id is empty string (use null)")
                elif not ORG_ID_RE.match(oid):
                    r.error(f"strict: work_experience[{i}].org_id '{oid}' invalid format")

        edu_allowed = EDU_FIELDS
        for i, edu in enumerate(data.get("education", [])):
            extra = set(edu.keys()) - edu_allowed
            if extra:
                r.error(f"strict: education[{i}] extra fields {sorted(extra)}")

        rel_allowed = REL_FIELDS
        for i, rel in enumerate(data.get("person_relationships", [])):
            if not (rel.get("relationship_type") or "").strip():
                r.error(f"strict: person_relationships[{i}].relationship_type is null/missing")
            extra = set(rel.keys()) - rel_allowed
            if extra:
                r.error(f"strict: person_relationships[{i}] extra fields {sorted(extra)}")
            rpid = rel.get("person_id")
            if isinstance(rpid, str):
                if not rpid.strip():
                    r.error(f"strict: person_relationships[{i}].person_id is empty string (use null)")
                elif rpid == pid:
                    r.error(f"strict: person_relationships[{i}] self-reference to own person_id")
                elif not PERSON_ID_RE.match(rpid):
                    r.error(f"strict: person_relationships[{i}].person_id '{rpid}' invalid format")
            nm = rel.get("person_name") or ""
            if _cjk_region_rules_apply() and nm:
                if not _has_cjk(nm):
                    r.error(f"strict: person_relationships[{i}].person_name '{nm}' has no Chinese (CJK region)")
                elif _bracket_reversed(nm):
                    r.error(f"strict: person_relationships[{i}].person_name '{nm}' bracket direction reversed (expected 中文 (English))")
...
def validate_cross_refs(base_dir):
    ...
    # Build index sets + keep data for strict identity checks
    org_ids_in_files = set()
    person_ids_in_files = set()
    registry_org_ids = set()
    registry_person_ids = set()
    org_data_map = {}      # org_id -> profile data
    person_data_map = {}   # person_id -> profile data
    org_name_map = {}      # org_id -> (name_zh, name_en)
    ...
    if os.path.isdir(orgs_dir):
        for fpath in glob.glob(os.path.join(orgs_dir, "*.json")):
            ...
            try:
                data = load_json_file(fpath)
                oid = data.get("org_id")
                org_ids_in_files.add(oid)
                org_data_map[oid] = data
                bi = data.get("basic_info") or {}
                org_name_map[oid] = ((bi.get("name_zh") or "").strip(),
                                     (bi.get("name_en") or "").strip())
            except Exception:
                pass
    ...
    if os.path.isdir(persons_dir):
        for fpath in glob.glob(os.path.join(persons_dir, "*.json")):
            ...
            try:
                data = load_json_file(fpath)
                pid = data.get("person_id")
                person_ids_in_files.add(pid)
                person_data_map[pid] = data
            except Exception:
                pass
        ...
        for fpath in sorted(glob.glob(os.path.join(orgs_dir, "*.json"))):
            ...
            for i, kp in enumerate(data.get("key_people", [])):
                ...
                if pid not in person_ids_in_files and pid not in registry_person_ids:
                    sev = "ERR" if STRICT else "WRN"
                    print(f"  {sev}: {fname} key_people[{i}].person_id '{pid}' not found in persons/ or registry")
                    if STRICT: errors += 1
                    else: warnings += 1
                elif pid not in person_ids_in_files and pid in registry_person_ids:
                    sev = "ERR" if STRICT else "WRN"
                    print(f"  {sev}: {fname} key_people[{i}].person_id '{pid}' in registry but no profile file")
                    if STRICT: errors += 1
                    else: warnings += 1
                elif STRICT:
                    # Identity sanity check: does the bound person profile show any trace
                    # of this org (org_id or org name in work_experience)? Catches
                    # surname/fuzzy-match mis-bindings.
                    pdata = person_data_map.get(pid)
                    if pdata is not None:
                        org_zh, org_en = org_name_map.get(data.get("org_id"), ("", ""))
                        hay = [((w.get("org_id") or ""), (w.get("organization") or ""))
                               for w in (pdata.get("work_experience") or [])]
                        hit = any(oid == data.get("org_id") for oid, _ in hay)
                        if not hit and org_zh:
                            hit = any(org_zh in nm for _, nm in hay)
                        if not hit and org_en:
                            oe = org_en.lower()
                            hit = any(oe and oe in nm.lower() for _, nm in hay)
                        if not hit:
                            print(f"  WRN(strict): {fname} key_people[{i}] '{kp.get('name')}' -> {pid}: "
                                  f"no trace of this org in person's work_experience (possible mis-binding)")
                            warnings += 1
            for i, re_ in enumerate(data.get("related_entities", [])):
                oid = re_.get("org_id")
                if not oid:
    ...
    for prid in registry_person_ids:
        if prid and prid not in person_ids_in_files:
            reg_entry = next((p for p in reg.get("person_registry", []) if p.get("person_id") == prid), {})
            status = reg_entry.get("status", "")
            if status in ("profiled", "listed"):
                sev = "ERR" if STRICT else "WRN"
                print(f"  {sev}: registry person '{prid}' (status={status}) has no file in persons/")
                if STRICT: errors += 1
                else: warnings += 1
...
def main():
    ...
    parser.add_argument("--strict", action="store_true", help="Data-quality gate: ironrule checks (sources, related org_id/relationship_type, key_people person_id, extra fields, empty-string IDs, self-refs) escalate to errors; cross-ref dangling refs become errors and mis-binding sanity check runs. CJK naming rules apply only to CN/HK/TW/MO datasets.")
    ...
    # Derive dataset ISO from path (…/output/<iso>/<date>/…) for region-aware rules
    m = re.search(r'[\\/]+([a-z]{2})[\\/]+', base_dir + os.sep)
    if m:
        DATASET_ISO = m.group(1).upper()
    if STRICT:
        print(f"=== STRICT MODE (dataset ISO: {DATASET_ISO or 'unknown'}; "
              f"CJK naming rules: {'on' if _cjk_region_rules_apply() else 'off'}) ===")
