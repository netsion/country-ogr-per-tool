...
Schema 文件：.agents/skills/country-org-collector/org_list_schema.json
...
步骤 1 — Wikidata 结构化查询（按 scope 选择模板）：
  ...
  SPARQL 模板详见：.agents/skills/country-org-collector/sparql_org_templates.md
...
```bash
# Step 1: QID 验证
python .agents/skills/country-org-collector/scripts/verify_qids.py output/{iso}/{date}

# Step 2: 批量数据采集（Wikidata EntityData + Wikipedia REST API）
python .agents/skills/country-org-collector/scripts/batch_collect.py output/{iso}/{date}

# Step 3: 从缓存生成骨架画像 + 解析 key_people QID 为姓名
python .agents/skills/country-org-collector/scripts/generate_profiles.py output/{iso}/{date}

# Step 4: 生成名称索引（P3/P4 子代理用于 ID 查找）
python .agents/skills/country-org-collector/scripts/generate_name_index.py output/{iso}/{date}
```
...
3. WHILE remaining 不为空:
   ...
   g. 运行脚本：
      python .agents/skills/country-org-collector/scripts/resolve_ids.py {output_dir} --allow-register --max-register 30
      python .agents/skills/country-org-collector/scripts/generate_name_index.py {output_dir}
...
写入后必须执行（不可跳过）：
1. 运行 `python .agents/skills/country-org-collector/scripts/validate_schema.py {output_dir} --file {filepath}`
2. 如有 ERRORS → 修复 → 重新验证 → 直到 0 errors
3. 运行 `python .agents/skills/country-org-collector/scripts/validate_schema.py {output_dir} --file {filepath} --score` 更新 completeness_score
...
1. 运行辅助脚本：
```bash
python .agents/skills/country-org-collector/scripts/update_person_list.py output/{iso}/{date}
```
...
2. WHILE remaining 不为空:
   ...
   f. 验证输出文件存在且 JSON 合法
   g. 每完成 1 个人物后运行：
      python .agents/skills/country-org-collector/scripts/resolve_ids.py output/{iso}/{date} --allow-register --max-register 30
      python .agents/skills/country-org-collector/scripts/generate_name_index.py output/{iso}/{date}
   h. 更新 _phase_progress.json
   i. 报告进度
...
第一步：读取必要文件（不可跳过）
1. 字段规则手册：.agents/skills/country-org-collector/person_collection_rules.md
2. Schema定义：.agents/skills/country-org-collector/person_profile_schema.json
...
第四步：写入后验证（不可跳过）
python .agents/skills/country-org-collector/scripts/validate_schema.py {output_dir} --file {filepath}
如有 ERRORS → 修复 → 重新验证 → 直到 0 errors。
python .agents/skills/country-org-collector/scripts/validate_schema.py {output_dir} --file {filepath} --score
...
python .agents/skills/country-org-collector/scripts/validate_schema.py output/{iso}/{date}
# 收尾必跑（数据质量门禁）：铁律类检查升级为 error（事件来源、关联组织 org_id/关系类型、
# key_people person_id、规则外字段、空字符串 ID、自引用、悬空引用、错绑健全性检查）。
# CJK 命名规范仅对 CN/HK/TW/MO 数据集生效。
python .agents/skills/country-org-collector/scripts/validate_schema.py output/{iso}/{date} --strict --cross-ref

# 自动修复 Unicode 弯引号等问题
python .agents/skills/country-org-collector/scripts/validate_schema.py output/{iso}/{date} --fix

# 自动规范化枚举值、日期格式、多余字段、韩文前置文本
python .agents/skills/country-org-collector/scripts/validate_schema.py output/{iso}/{date} --normalize

# 跨文件交叉引用验证（P5 收尾时必做）
python .agents/skills/country-org-collector/scripts/validate_schema.py output/{iso}/{date} --cross-ref
```
...
```bash
python .agents/skills/country-org-collector/scripts/generate_index.py output/{iso}/{date}
```
...
| `generate_name_index.py` | P2/P3/P4 | 名称→ID 索引生成（含多语言分解），子代理用于 ID 查找 |
| `resolve_ids.py` | P3/P4 | 批量解析 null ID → 已知 ID（多语言分解匹配）；未匹配项**默认保持 null 并报告**，仅在显式给出 `--allow-register` 时才注册新 ID（`--max-register` 限流、`--allow-register-work-orgs` 控制 work 经历组织、计数器自动对齐磁盘防撞号） |
| `atomic_write.py` | 共享 | 原子写入 + `normalize_name()` + `decompose_name()`（中/韩/英名称分解） |
